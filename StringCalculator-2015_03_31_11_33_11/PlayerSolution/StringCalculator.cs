...
namespace PlayerStringKata
{
    public class StringCalculator : IStringCalculator
    {
        ...
        private static string StringSanitizer(string input) {
            ...
            var customedelimiter = removebrackets.Split(new[] {','}, StringSplitOptions.RemoveEmptyEntries);


            for (int i = 0; i < customedelimiter.Length; i++)
            {
                input = input.Replace(customedelimiter[i], ",");
            }
            return input;
        }
        ...
        private static void ThrowNegativesException(List<int> convertedNumbers) {
            List<int> Negatives = convertedNumbers.Where(x => x < 0).ToList();
            if (Negatives.Any())
            {
